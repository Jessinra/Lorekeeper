...
@pytest.fixture
def mcp_handler(svc):
    """MCPHandler wired with all 5 processors from the built App."""
    return MCPHandler(
        memory_processor=svc.memory_processor,
        suggestion_processor=svc.suggestion_processor,
        reflection_processor=svc.reflection_processor,
        link_processor=svc.link_processor,
        admin_processor=svc.admin_processor,
    )


def test_handle_insert_missing_title_includes_index_in_message(svc, mcp_handler):
...
def test_handle_insert_valid_memory_succeeds(svc, mcp_handler):
    result = mcp_handler.insert(
...
def test_handle_insert_no_memories(svc, mcp_handler):
    """Empty memories list should succeed with no inserted items."""
    result = mcp_handler.insert(memories=[], links=[])
    assert result["inserted_memories"] == []
    assert result["errors"] == []


def test_handle_insert_invalid_inline_link_format(svc, mcp_handler):
...
def test_search_refine_from_exceeds_cap(svc, mcp_handler):
    ...
    with pytest.raises(ValueError, match="refine_from exceeds cap of 200 IDs"):
        mcp_handler.search(query="test", refine_from=oversize)


def test_search_refine_from_empty_is_noop(svc, mcp_handler):
    """refine_from with an empty list should succeed (no filtering)."""
    svc.write_service.insert(
        memories=[{"title": "m1", "content": "one"}],
        links=[],
    )
    result = mcp_handler.search(query="test", refine_from=[])
...
def test_search_title_format_returns_compact_results(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="alpha", format="title")
...
def test_search_title_format_backward_compatible(svc, mcp_handler):
    ...
    all_rows = svc.memories.all_memory_rows()
    svc._engine._search_results = [{"lore_id": r["id"], "score": 0.9} for r in all_rows]
    result = mcp_handler.search(query="gamma")
    assert "results" in result
    for item in result["results"]:
...
def test_search_title_format_with_empty_results(svc, mcp_handler):
    """format='title' with no matches returns empty results."""
    result = mcp_handler.search(query="nonexistent_zzz", format="title")
...
def test_search_by_ids_returns_matching_memories(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="", ids=target_ids)
    assert len(result["results"]) == 2
    returned_ids = {item["memory"]["id"] for item in result["results"]}
    assert set(target_ids) == returned_ids


def test_search_by_ids_empty_list_returns_empty(svc, mcp_handler):
    """Empty ids list returns no results."""
    result = mcp_handler.search(query="", ids=[])
    assert result["results"] == []


def test_search_by_ids_nonexistent_silently_ignored(svc, mcp_handler):
    """Non-existent IDs in ids list are silently skipped."""
    result = mcp_handler.search(query="", ids=["nonexistent-id"])
    assert result["results"] == []


def test_search_by_ids_with_title_format(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="", ids=[target_id], format="title")
    assert len(result["results"]) == 1
...
def test_search_by_ids_exceeds_cap_raises(svc, mcp_handler):
    ...
    with pytest.raises(ValueError, match="ids exceeds cap of 50 IDs"):
        mcp_handler.search(query="query", ids=oversize)


# ── empty query guard ─────────────────────────────────────────────────────────


def test_search_empty_query_without_ids_raises(svc, mcp_handler):
    """Empty query with no ids raises ValueError."""
    with pytest.raises(ValueError, match="query is required"):
        mcp_handler.search(query="")


def test_search_blank_query_without_ids_raises(svc, mcp_handler):
    """Whitespace-only query with no ids raises ValueError."""
    with pytest.raises(ValueError, match="query is required"):
        mcp_handler.search(query="   ")


def test_search_empty_query_with_ids_succeeds(svc, mcp_handler):
...
def test_search_by_ids_include_links_fetches_actual_links(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="", ids=[ids[0]], include_links=True)
    assert len(result["results"]) == 1
    item = result["results"][0]
...
def test_search_by_ids_increments_usage_count(svc, mcp_handler):
    ...
    mcp_handler.search(query="", ids=[mem_id])
...
def test_search_by_ids_dedup_with_usage_count(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="", ids=[mem_id, mem_id, mem_id])
...
# ── recommend_links input validation ─────────────────────────────────


def test_recommend_links_empty_lore_id_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="lore_id is required"):
        mcp_handler.recommend_links(lore_id="")


def test_recommend_links_whitespace_lore_id_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="lore_id is required"):
        mcp_handler.recommend_links(lore_id="   ")


def test_recommend_links_zero_top_k_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="positive integer"):
        mcp_handler.recommend_links(lore_id="abc", top_k=0)


def test_recommend_links_negative_top_k_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="positive integer"):
        mcp_handler.recommend_links(lore_id="abc", top_k=-5)


def test_recommend_links_top_k_capped_at_50(svc, mcp_handler):
    """top_k > 50 must be silently capped — no error, result count <= 50."""
    with patch.object(
        svc.suggestion_service,
        'recommend_links',
        wraps=svc.suggestion_service.recommend_links,
    ) as mock_domain:
        result = mcp_handler.recommend_links(lore_id="nonexistent-id", top_k=999)
...
def test_recommend_links_valid_call_returns_shape(svc, mcp_handler):
    """Valid call must return dict with candidates, count, source_lore_id.

    Invariant: count must equal len(candidates).
    """
    result = mcp_handler.recommend_links(lore_id="some-id")
...
def test_created_after_invalid_iso_string_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="Invalid ISO timestamp for 'created_after'"):
        mcp_handler.search(query="test", created_after="not-a-date")


def test_updated_after_invalid_iso_string_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="Invalid ISO timestamp for 'updated_after'"):
        mcp_handler.search(query="test", updated_after="2026/06/01")


def test_created_after_non_utc_offset_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="Non-UTC timezone offset"):
        mcp_handler.search(query="test", created_after="2026-06-01T00:00:00+05:30")


def test_created_after_utc_z_notation_accepted(svc, mcp_handler):
    """Z suffix is UTC — should not raise."""
    result = mcp_handler.search(query="test", created_after="2026-06-01T00:00:00Z")
    assert "results" in result


def test_created_after_naive_string_treated_as_utc(svc, mcp_handler):
    """Naive ISO strings (no tz) are treated as UTC — should not raise."""
    result = mcp_handler.search(query="test", created_after="2026-06-01T00:00:00")
    assert "results" in result


def test_created_after_plus_zero_utc_accepted(svc, mcp_handler):
    """+00:00 offset is UTC — should not raise."""
    result = mcp_handler.search(query="test", created_after="2026-06-01T00:00:00+00:00")
    assert "results" in result


def test_created_after_filters_in_full_pipeline(svc, mcp_handler):
    ...
    result = mcp_handler.search(query="content", created_after="2026-03-01T00:00:00")
...
def test_sort_by_unknown_value_raises(svc, mcp_handler):
    with pytest.raises(ValueError, match="Unknown sort_by"):
        mcp_handler.search(query="test", sort_by="magic")


def test_sort_by_valid_values_do_not_raise(svc, mcp_handler):
    """All three valid sort_by values must be accepted without error."""
    for valid in ("relevance", "recent", "frequent"):
        result = mcp_handler.search(query="test", sort_by=valid)
        assert "results" in result


def test_sort_by_recent_returns_results(svc, mcp_handler):
    ...
    all_rows = svc.memories.all_memory_rows()
    svc._engine._search_results = [{"lore_id": r["id"], "score": 0.9} for r in all_rows]
    result = mcp_handler.search(query="content r1", sort_by="recent")
    assert isinstance(result["results"], list)


def test_sort_by_frequent_returns_results(svc, mcp_handler):
    ...
    all_rows = svc.memories.all_memory_rows()
    svc._engine._search_results = [{"lore_id": r["id"], "score": 0.9} for r in all_rows]
    result = mcp_handler.search(query="content f1", sort_by="frequent")
    assert isinstance(result["results"], list)


def test_sort_by_and_created_after_compose_in_handler(svc, mcp_handler):
...
class TestSuggestionHandlers:
    ...
    @pytest.fixture
    def suggestion_handler(self, suggestion_svc):
        """MCPHandler wired with suggestion-specific App processors."""
        return MCPHandler(
            memory_processor=suggestion_svc.memory_processor,
            suggestion_processor=suggestion_svc.suggestion_processor,
            reflection_processor=suggestion_svc.reflection_processor,
            link_processor=suggestion_svc.link_processor,
            admin_processor=suggestion_svc.admin_processor,
        )

    # ── get_suggestions input validation ──────────────────────────────

    def test_get_suggestions_zero_limit_raises(self, suggestion_handler):
        with pytest.raises(ValueError, match="limit must be a positive integer"):
            suggestion_handler.get_suggestions(limit=0)

    def test_get_suggestions_negative_limit_raises(self, suggestion_handler):
        with pytest.raises(ValueError, match="limit must be a positive integer"):
            suggestion_handler.get_suggestions(limit=-1)

    def test_get_suggestions_bool_limit_raises(self, suggestion_handler):
        """bool is a subclass of int — True/False must be rejected."""
        with pytest.raises(ValueError, match="limit must be a positive integer"):
            suggestion_handler.get_suggestions(limit=True)

    def test_get_suggestions_min_score_out_of_range_raises(
        self, suggestion_handler, suggestion_stores
    ):
        with pytest.raises(ValueError, match="min_score must be between"):
            suggestion_handler.get_suggestions(min_score=1.5)

    def test_get_suggestions_negative_min_score_raises(
        self, suggestion_handler, suggestion_stores
    ):
        with pytest.raises(ValueError, match="min_score must be between"):
            suggestion_handler.get_suggestions(min_score=-0.1)

    # ── get_suggestions happy path ───────────────────────────────────

    def test_get_suggestions_returns_pending_only(self, suggestion_handler, suggestion_stores):
        ...
        result = suggestion_handler.get_suggestions()
    ...
    def test_get_suggestions_limit_caps_results(self, suggestion_handler, suggestion_stores):
        for i in range(5):
            self._make_suggestion(
                suggestion_stores, src_id=f"s{i}", tgt_id=f"t{i}", score=0.5 + i * 0.1
            )
        result = suggestion_handler.get_suggestions(limit=3)
        assert result["count"] == 3
        assert result["total_pending"] == 5  # total not affected by limit

    def test_get_suggestions_min_score_filters(self, suggestion_handler, suggestion_stores):
        ...
        result = suggestion_handler.get_suggestions(min_score=0.5)
        assert result["count"] == 1
        assert result["suggestions"][0]["source_memory_id"] == "hi"

    def test_get_suggestions_response_shape(self, suggestion_handler, suggestion_stores):
    ...
    # ── review_suggestion input validation ────────────────────────────

    def test_review_empty_ids_raises(self, suggestion_handler, suggestion_stores):
        with pytest.raises(ValueError, match="suggestion_ids must not be empty"):
            suggestion_handler.review_suggestion(
                suggestion_ids=[], action="accept")

    def test_review_invalid_action_raises(self, suggestion_handler, suggestion_stores):
        with pytest.raises(ValueError, match="action must be 'accept' or 'reject'"):
            suggestion_handler.review_suggestion(
                suggestion_ids=["some-id"], action="approve")

    def test_review_whitespace_only_ids_raises(self, suggestion_handler, suggestion_stores):
        with pytest.raises(ValueError, match="suggestion_ids contained only empty strings"):
            suggestion_handler.review_suggestion(
                suggestion_ids=["   ", ""], action="accept")

    # ── review_suggestion accept path ────────────────────────────────

    def test_review_accept_creates_link(
        self, suggestion_svc, suggestion_handler, suggestion_stores
    ):
        """Accepting a suggestion creates a real memory_links row."""
        sug = self._make_suggestion(suggestion_stores)
        result = suggestion_handler.review_suggestion(
    ...
    def test_review_accept_updates_suggestion_status(self, suggestion_handler, suggestion_stores):
        sug = self._make_suggestion(suggestion_stores)
        suggestion_handler.review_suggestion(
            suggestion_ids=[sug.id], action="accept",
        )
    ...
    def test_review_accept_unknown_id_skipped(self, suggestion_handler, suggestion_stores):
        result = suggestion_handler.review_suggestion(
    ...
    # ── review_suggestion reject path ────────────────────────────────

    def test_review_reject_updates_suggestion_status(self, suggestion_handler, suggestion_stores):
        sug = self._make_suggestion(suggestion_stores)
        result = suggestion_handler.review_suggestion(
    ...
    def test_review_reject_unknown_id_skipped(self, suggestion_handler, suggestion_stores):
        result = suggestion_handler.review_suggestion(
    ...
    def test_review_batch_accept_multiple(self, suggestion_handler, suggestion_stores):
