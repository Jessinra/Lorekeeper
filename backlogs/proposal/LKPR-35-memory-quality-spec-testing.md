...
## Problem
No observability into whether memory extraction is working correctly. The agent stores facts all session — but there's no way to validate it stored the *right* ones. You only notice issues when retrieval fails in production.
...
test-user-timezone:
  ...
  expect:
    lore_search("user timezone")[0].content ~= "PST"
