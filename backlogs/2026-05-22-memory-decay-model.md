...
## Acceptance Criteria
- [ ] Decay multiplier is applied during re-ranking in `search.py`
- [ ] `LORE_DECAY_LAMBDA` is documented in `config.py` with a sensible default (e.g. half-life ~90 days for unused memories)
- [ ] `README.md` scoring formula section updated
- [ ] Decay does not affect stored scores — applied only at query time
- [ ] Optional: `decay_factor` exposed in search result output
