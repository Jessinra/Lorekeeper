...
- **Double-check the base SHA.** If `<base-sha>` is wrong and you rewrite commits from main, you'll corrupt the branch. Always confirm with `git log <base-sha> --oneline -3` that it's the right commit before running filter-branch.
