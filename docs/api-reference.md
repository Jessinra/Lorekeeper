...
`total_pending` gives agents a sense of workload without a separate count call. Only `status='pending'` suggestions are returned; `min_score` filters within that set by `weighted_score`.
