...
import { clearFilter, toggleShowDeleted, setMemSort, setNamespaceFilter, setTimeFilter } from "./memories.js";
...
document.addEventListener("click", (e) => {
  ...
  if (action) {
    ...
    if (cmd === "clear-filter") { clearFilter(); return; }
    if (cmd === "toggle-deleted") { toggleShowDeleted(); return; }
    if (cmd === "refresh") { triggerRefresh(); return; }
    // etc.
  }
  const tabBtn = e.target.closest("[data-tab]");
...
4. **Each tab module**: self-registers, no window.*, uses data-* in templates
