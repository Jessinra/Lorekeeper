...
def _migration_5_add_link_suggestions(conn: sqlite3.Connection) -> None:
    """Add link_suggestions table for the sweep engine (LKPR-99).

    Stores candidate link pairs generated by the sweep, with scores and
    confidence labels. No auto-accept — suggestions are for agent review.
    Idempotent via IF NOT EXISTS on both table and index.
    """
    conn.executescript("""
...
MIGRATIONS: list[tuple[int, str, Callable[[sqlite3.Connection], None]]] = [
    ...
    (5, "add_link_suggestions_table", _migration_5_add_link_suggestions),
]
