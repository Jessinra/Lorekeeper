...
import { esc, fmtDate, fmtDatePlus8, fmtRelative, htmlSection, clientSort } from './utils.js';
...
export async function loadSessions(force = false) {
  if (_loaded && !force) { renderSessions(); return; }
...
function renderSessions() {
  ...
  document.getElementById('session-rows').innerHTML = visible.length === 0
...
function renderTaskDist(sessions) {
  ...
  el.querySelectorAll('.sess-dist-row').forEach(row => {
    row.addEventListener('click', () => {
      ...
      window.filterByTask(document.querySelector(`.sess-chip[data-filter="${task}"]`), task);
    });
  });
}
...
function renderRow(s, i) {
  ...
  const summary   = s.what_was_done
    ? esc(s.what_was_done.replace(/\n/g, ' ').slice(0, 130)) + (s.what_was_done.length > 130 ? '…' : '')
...
function renderDetail(s) {
  if (!s.what_was_done && !s.decisions && !s.lessons_learnt && !s.good_patterns && !s.user_profile && !s.discoveries) {
...
window.toggleSessDetail = function(detId) {
  const el = document.getElementById(detId);
  if (el) el.classList.toggle('hidden');
};

window.toggleHideStubs = function() {
  _hideStubs = !_hideStubs;
  renderSessions();
};

window.filterByTask = function(btn, task) {
  _filterTask = task;
  document.querySelectorAll('.sess-chip').forEach(c => c.classList.remove('active'));
  if (btn) btn.classList.add('active');
  renderSessions();
};

window.filterBySessionId = function(val) {
  _filterSessId = val;
  renderSessions();
};
