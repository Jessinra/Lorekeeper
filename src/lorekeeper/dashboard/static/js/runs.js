...
export async function loadRuns(force = false) {
  if (_loaded && !force) { renderRuns(); return; }
...
function renderRuns() {
  ...
  document.getElementById('run-met-last').textContent     = runs.length ? fmtDate(runs[0].completed_at) : '—';
  document.getElementById('runs-status').textContent      = runs.length ? `${runs.length} runs recorded` : '';

  document.getElementById('run-rows').innerHTML = runs.length === 0
...
function renderRow(r, i) {
  ...
  const insList = (r.lore_inserted     ?? []).map(m => `<div class="run-mem-row run-mem-ins">+ ${esc(m.title)}</div>`).join('');
  const updList = (r.lore_updated      ?? []).map(m => `<div class="run-mem-row run-mem-upd">~ ${esc(m.title)}</div>`).join('');
  const delList = (r.lore_soft_deleted ?? []).map(m => `<div class="run-mem-row run-mem-del">× ${esc(m.title)}</div>`).join('');
  ...
  return `
    ...
    ${hasDetail ? `<tr id="${detId}" class="run-detail-row hidden">
