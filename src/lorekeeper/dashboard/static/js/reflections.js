...
export async function loadReflections(force = false) {
  if (_loaded && !force) { renderReflections(); return; }
...
function renderReflections() {
  ...
  document.getElementById('ref-met-last').textContent     = refs.length ? fmtDate(refs[0].created_at) : '—';
  document.getElementById('ref-status').textContent       = refs.length ? `${refs.length} reflections` : '';

  document.getElementById('reflection-rows').innerHTML = refs.length === 0
...
function renderRow(r, i) {
  ...
  const summary  = r.summary ? esc(r.summary.slice(0, 120)) + (r.summary.length > 120 ? '…' : '') : '—';
...
window.toggleReflectionDetail = async function(detId, reflectionId) {
...
function renderDetail(data) {
  ...
  const memLinks = memIds.length
    ? `<div class="ref-section"><div class="ref-section-title">Memories Created</div><div class="ref-section-body">${
        memIds.map(id => `<span class="ref-mem-link" onclick="window._refGoToMemory('${esc(id)}')">${esc(id.slice(0, 8))}…</span>`).join(' ')
      }</div></div>`
    : '';

  const sessionList = sessions.length
    ? `<div class="ref-section"><div class="ref-section-title">Sessions Covered</div><div class="ref-section-body">${
        sessions.map(s => {
...
export function registerRefGoToMemory(fn) { _goToMemory = fn; }
window._refGoToMemory = (id) => { if (_goToMemory) _goToMemory(id); };
