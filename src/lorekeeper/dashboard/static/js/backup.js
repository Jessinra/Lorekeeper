...
function _renderPreview(data) {
  ...
  for (const m of mems) {
    const desc = m.description ? `<div style="font-size:12px;color:var(--subtle);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(m.description)}</div>` : '';
    const tag  = m.type ? `<span style="font-size:10px;padding:1px 5px;border-radius:3px;background:var(--chip-bg,#2a2a3a);color:var(--subtle);margin-left:6px">${esc(m.type)}</span>` : '';
  ...
  for (const lnk of links) {
    ...
    const reason = lnk.reason ? `<div style="font-size:12px;color:var(--subtle);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(lnk.reason)}</div>` : '';
    rows.push(`<div class="bk-row" style="padding:7px 12px;border-bottom:1px solid var(--border)">
