...
import { esc, scoreClass, fmt2, fmtDate, isToday, clientSort } from './utils.js';

// Cross-module callback — wired by app.js to avoid circular imports.
// memories.js needs to call selectMemory (lives in detail.js).
let _selectMemory = () => {};
export function registerSelectMemory(fn) { _selectMemory = fn; }
...
export function updateStats() {
  ...
  avgEl.className   = 'metric-value ' + (
    avgScore == null ? '' : avgScore >= 6 ? 'stat-high' : avgScore >= 3 ? 'stat-mid' : 'stat-low'
  );
...
export function onFilterInput() {
  ...
  document.getElementById('mem-filter-clear').classList.toggle('hidden', !state.filterText);
...
export function setMemSort(field) {
  state.memSort.dir = state.memSort.field === field
    ? (state.memSort.dir === 'desc' ? 'asc' : 'desc')
    : 'desc';
...
export function renderList() {
  ...
  const countLabel = (ft || state.timeFilterDays !== null)
  ...
  document.getElementById('memory-rows').innerHTML = sorted.map(m => {
    const cls  = [state.selectedId === m.id ? 'selected' : '', m.soft_deleted ? 'deleted-row' : ''].filter(Boolean).join(' ');
    const conf = m.confidence != null ? fmt2(m.confidence) : '—';
    const newDot = isToday(m.created_at) ? '<span class="new-dot" title="Created today"></span>' : '';
    const sub  = m.description ? `<div class="col-title-sub">${esc(m.description)}</div>` : '';
