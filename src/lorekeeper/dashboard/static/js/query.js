...
let _selectMemory = () => {};
export function registerQuerySelectMemory(fn) { _selectMemory = fn; }
...
export async function runQuery() {
  ...
  try {
    const results = await api('POST', '/api/search', { query, limit, min_score: minScore });
    if (!results.length) { el.innerHTML = '<div class="no-results">No results.</div>'; return; }
